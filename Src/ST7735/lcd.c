...
static int32_t lcd_writereg(uint8_t reg, uint8_t* pdata, uint32_t length)
{
  if (length + 1 > SPI_DMA_MAX_PAYLOAD) {
    led_set_interval(LED_ERROR_INTERVAL);
    return -1;
  }

  // Send command byte (RS low, CS stays active if data follows)
  display_spi_enqueue(&reg, 1, false, (length == 0));

  // Send data bytes if any (RS high, release CS after final transfer)
  if (pdata && length > 0)
    display_spi_enqueue(pdata, length, true, true);
...
static int32_t lcd_readreg(uint8_t reg, uint8_t* pdata)
{
  // Keep this blocking, as reads are rare and short
...
static int32_t lcd_senddata(uint8_t* pdata, uint32_t length)
{
  ...
  // Send raw pixel/data stream (RS high, release CS after)
  display_spi_enqueue(pdata, length, true, true);
  return 0;
}
...
static int32_t lcd_recvdata(uint8_t* pdata, uint32_t length)
{
  // Keep blocking for simplicity (used infrequently)
  display_spi_cs_reset();
